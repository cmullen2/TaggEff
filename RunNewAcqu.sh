...
FILES=`ls -lh --time-style=+'%Y%m%d%H%M%S' scratch/ | grep CBTaggTAPS | awk -v time="$TIME" '{if ($6 < time) print "scratch/"$7}'`
...
FILES=`ls -lh --time-style=+'%Y%m%d%H%M%S' scratch/ | grep TaggEff | awk -v time="$TIME" '{if ($6 < time) print "scratch/"$7}'`
