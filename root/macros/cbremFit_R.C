...
void enhFromHuman(Double_t beamMeV = 1557.0, Double_t edgeMeV = 350.0, Double_t spreadMeV = 10.0, 
		  Double_t colliDist_m = 2.5, Double_t colliRad_mm = 1.0, Int_t nVec = 4);
void parFromHuman(Double_t beamMeV = 1557.0, Double_t edgeMeV = 350.0, Double_t spreadMeV = 10.0, 
		  Double_t colliDist_m = 2.5, Double_t colliRad_mm = 1.0, Int_t nVec = 4, Double_t *par=NULL);
...
Double_t LOWFIT = 500.0;         //how far below the peak in MeV to start fitting

//All histograms and related are globals
TH1F *histP = NULL;  //pol
TH1F *histE = NULL;  //enh from calculation
TH1F *histR = NULL;  //enh from data (raw)
...
void fitData(Char_t *fname, Char_t *outfile, Double_t beamMeV, Double_t colliDist_m, Double_t colliRad_mm, Int_t nVec){
  ...
  Char_t line[250];
  Double_t diff1,diff2,diff3,fitedge,scalefac;
  Char_t name[30];
  ...
  for(int f=0;f<nFiles;f++){                         //go over the all data files    
	...
	energyBins[b+1]=0.5*(energy[b]+energy[b+1]);
      ...
      histD        = new TH1F("EnhancementData", filename[f] ,nBins,energyBins);
      histR        = new TH1F("EnhancementDataRaw", filename[f] ,nBins,energyBins);
    ...
    //  //cout << "Hist integral = " << histD->Integral(n-6,n+6) << endl;
    //  //cout << "Bin content = " << histD->GetBinContent(n) << endl;
    //  if(histD->GetBinContent(n)>10){
    //    //cout << "Bin content larger than 10: Bin = " << n << " content = " << histD->GetBinContent(n) << endl;
    //	histD->SetBinContent(n,1);
    //    //histD->SetBinContent(n,0.5*(histD->GetBinContent(n-1)+histD->GetBinContent(n+1)));
    //  }
    //  //cout << "Bin content after = " << histD->GetBinContent(n) << endl;
    //  
    //}
    
    
    //  Get rid of spikes up and down
    //for(sysNum=0.1; sysNum>0.01; sysNum-=0.01){              // Loops over limit for 'smoothing' spikes
    
      for(Double_t funNum=1; funNum>0.05; funNum-=0.01){     // Sets limit for 'smoothing' spikes
      //for(Double_t funNum=1; funNum>sysNum; funNum-=0.01){   // Sets limit for 'smoothing' spikes (when sysnum loop included)
	...
	Int_t oldN = 0;
	  ...
	  if(n==oldN) break;
          ...
          diff3=(histD->GetBinContent(n+1)-histD->GetBinContent(n+2))/histD->GetBinContent(n+2);
          //diff1=(histD->GetBinContent(n)-histD->GetBinContent(n-1));
          //diff2=(histD->GetBinContent(n)-histD->GetBinContent(n+1));
          //diff3=(histD->GetBinContent(n+1)-histD->GetBinContent(n+2));
          if(((fabs(diff1)>funNum)&&(fabs(diff2)>funNum))||((fabs(diff1)>funNum)&&(fabs(diff3)>funNum))&&(fabs(diff1-diff2)>0.2)){
	    cout << "****" << endl;
	    histD->SetBinContent(n,0.5*(histD->GetBinContent(n-1)+histD->GetBinContent(n+1)));
	  }
	  oldN = n;
      ...
      Int_t backBad = 0;
      Double_t content = 0;
      Double_t oldContent = 0;
      
      //      for(int n=2;n<=nBins-1;n++){ // May need to put to nBins-2 as below uses n+2
      for(int n=2;n<=nBins-1;n++){ // May need to put to nBins-2 as below uses n+2
	
        //if(n>=histD->GetMaximum()-10 && n<=histD->GetMaximum()+10){ // Loops attempting to leave 'smoothing' from the coh.peak region
        //  cout << n << " = N" << endl;
        //  continue;
        //}
        //else{
        ...
        //if((histD->GetBinContent(n)>0.0)&&(histD->GetBinContent(n-1)>0.0)&&(histD->GetBinContent(n+1)>0.0)){  // ######### TEST #######
        
        //if (((fabs(diff1)>0.1)&&(fabs(diff2)>0.1))&&(fabs(diff1-diff2)<1.5)){  // #### TRY CHANGING VALUES HERE ######
        //	if (((fabs(diff1)>funNum*histD->GetBinContent(n))&&(fabs(diff2)>funNum*histD->GetBinContent(n)))&&(fabs(diff1-diff2)<0.1)){
        if (((fabs(diff1)>funNum)&&(fabs(diff2)>funNum))||((fabs(diff1)>funNum)&&(fabs(diff3)>funNum))&&(fabs(diff1-diff2)>0.2)){
          //cout << "****" << endl;
          content = 0.5*(histD->GetBinContent(n-1)+histD->GetBinContent(n+1));
          histD->SetBinContent(n,0.5*(histD->GetBinContent(n-1)+histD->GetBinContent(n+1)));
	...
	//} // Check bin, the bin before and bin after actually have non-zero content
      }
    ...
    // ####### Use below section when running over sysNum loop to see output from changing smoothing limit #######
    
    //sprintf(tempname,"%s_Sys-%f.gif",filename[f],sysNum);
    //genCanvas->SaveAs(tempname);
    //if(outfile==NULL){
    //  sprintf(tempname,"%s_Sys-%f.table",filename[f],sysNum);
    //  fp=fopen(tempname,"w");
    //}
    //else{
  ...
  for(int f=0;f<nFiles;f++){
    ...
    genCanvas->SaveAs(tempname);
    sprintf(tempname,"%s.root",filename[f]);
    histP->SaveAs(tempname);
    sprintf(tempname,"%s.R.root",filename[f]);
    TFile *OutRFile = new TFile(tempname,"RECREATE");
    histR->Write();
    histP->Write();
    histD->Write();
    histE->Write();
    ...
    //Print out errors on parameters                      ####Parameter errors ####
	...
	if(gausFit->Derivative(d)<lowmean){
      ...
      for(int b=0;b<nBins;b++){                                             //go over all the bins            #### BIN POLARISATION ####
